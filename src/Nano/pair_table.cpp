  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        if (rsq < tb->innersq){
	  fprintf(screen,"Error at timestep %d on particles %d and %d\n",update->ntimestep ,atom->tag[i],atom->tag[j]);
	  fprintf(screen,"Distance: %e, allowed distance: %e\n",sqrt(rsq),sqrt(tb->innersq));
	  fprintf(screen,"Coordinates %d: %e %e %e\n",atom->tag[i],x[i][0],x[i][1],x[i][2]);
	  fprintf(screen,"Coordinates %d: %e %e %e\n",atom->tag[j],x[j][0],x[j][1],x[j][2]);
