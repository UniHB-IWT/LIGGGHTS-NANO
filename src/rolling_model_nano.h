...
#ifdef ROLLING_MODEL
ROLLING_MODEL(ROLLING_NANO,nano,5)
#else
#ifndef ROLLING_MODEL_NANO_H_
#define ROLLING_MODEL_NANO_H_
...
namespace ContactModels
{
  ...
  template<>
  class RollingModel<ROLLING_NANO> : public RollingModelBase {
  public:
    ...
    void connectToProperties(PropertyRegistry & registry)
    {
      ...
      registry.connect("coeffRollFrict", coeffRollFrict,"rolling_model nano");

      // error checks on coarsegraining
      if(force->cg_active())
        error->cg(FLERR,"rolling model nano");
    }
    ...
    void surfacesIntersect(SurfacesIntersectData & sidata, ForceData & i_forces, ForceData & j_forces)
    {
      ...
      if(sidata.is_wall){
        ...
        if(wr_rollmag > 0.)
        {
          ...
          // ###################################################################
	  // LIGGGHTS-NANO
          // calculate area dependence of the friction force.
          const double radi = sidata.radi;
          const double radj = sidata.radj;
