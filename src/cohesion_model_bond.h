...
COHESION_MODEL(COHESION_BOND,bond,9)
#else
...
{
    inline static MatrixProperty* createBondYoungs(PropertyRegistry & registry, const char * caller, bool sanity_checks)
    {
      return createPerTypePairProperty(registry, "bondYoungs", caller);
    }

    inline static MatrixProperty* createBondPoisson(PropertyRegistry & registry, const char * caller, bool sanity_checks)
    {
      return createPerTypePairProperty(registry, "bondPoisson", caller);
    }

    inline static MatrixProperty* createBondTensileStrength(PropertyRegistry & registry, const char * caller, bool sanity_checks)
    {
      return createPerTypePairProperty(registry, "bondTensileStrength", caller);
    }

    inline static MatrixProperty* createBondShearStrength(PropertyRegistry & registry, const char * caller, bool sanity_checks)
    {
      return createPerTypePairProperty(registry, "bondShearStrength", caller);
    }

    inline static MatrixProperty* createBondRadius(PropertyRegistry & registry, const char * caller, bool sanity_checks)
    {
      return createPerTypePairProperty(registry, "bondRadius", caller);
    }
    inline static MatrixProperty* createBondMaxSeparationDistance(PropertyRegistry & registry, const char * caller, bool sanity_checks)
    {
...
namespace ContactModels {
  ...
  public:
        ...
        history_contact(0)
    {
      ...
      hsetup->add_history_value("initialLength","0");
       if(cmb->is_wall())
        error->warning(FLERR,"The cohesion model bond does not support walls");
    }

    void registerSettings(Settings& settings)
    {
      settings.registerOnOff("per_molecule",perMolecule_,true);
      settings.registerOnOff("full_length",fullLength_,false);
      settings.registerOnOff("write_file",writeFile_,false);
    }

    inline void postSettings(IContactHistorySetup * hsetup, ContactModelBase *cmb) {}
    void connectToProperties(PropertyRegistry & registry) {
      registry.registerProperty("bondYoungs", &MODEL_PARAMS::createBondYoungs);
      registry.registerProperty("bondPoisson", &MODEL_PARAMS::createBondPoisson);
      registry.registerProperty("bondTensileStrength", &MODEL_PARAMS::createBondTensileStrength);
      registry.registerProperty("bondShearStrength", &MODEL_PARAMS::createBondShearStrength);
      registry.registerProperty("bondRadius", &MODEL_PARAMS::createBondRadius);
      registry.registerProperty("bondMaxSeparationDistance", &MODEL_PARAMS::createBondMaxSeparationDistance);

      registry.connect("bondYoungs", bondYoungs,"cohesion_model bond");
      registry.connect("bondPoisson", bondPoisson,"cohesion_model bond");
      registry.connect("bondTensileStrength", bondTensileStrength,"cohesion_model bond");
      registry.connect("bondShearStrength", bondShearStrength,"cohesion_model bond");
      registry.connect("bondRadius", bondRadius,"cohesion_model bond");
      registry.connect("bondMaxSeparationDistance", bondMaxSeparationDistance,"cohesion_model bond");

      // error checks on coarsegraining
      if(force->cg_active())
        error->cg(FLERR,"cohesion model bond");
    ...
    void surfacesIntersect(SurfacesIntersectData & sidata, ForceData & i_forces, ForceData & j_forces)
    {
      ...
      */
      ...
      double rbond = bondRadius[sidata.itype][sidata.jtype];
      // Bond youngs modulus and poisson ratio
      double youngs = bondYoungs[sidata.itype][sidata.jtype];
      double poisson = bondPoisson[sidata.itype][sidata.jtype];
      ...
      /////////////////////////////////////////////////////////////////
      // 1. criterion: Definition of a bond
      ...
      const double Sn = (youngs) / (historyvalues[14]);
      const double St = (youngs) / (2 * (historyvalues[14]) * (1 + (poisson)));
    ...
    void surfacesClose(SurfacesCloseData & scdata, ForceData & i_forces, ForceData & j_forces)
    {
      ...
      */
      ...
      // The values for the bond radius
      double rbond = bondRadius[scdata.itype][scdata.jtype];
      // Bond youngs modulus and poisson ratio
      double youngs = bondYoungs[scdata.itype][scdata.jtype];
      double poisson = bondPoisson[scdata.itype][scdata.jtype];
      // Bond values for tensile strength
